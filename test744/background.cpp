...
Backgrounds::Backgrounds(sf::Texture* texture, float speed, int number, sf::Vector2f sizes, sf::Vector2f positions)
{
	this->number = number;
	this->speed = speed;
	this->size = sizes;
...
void Backgrounds::Update(float deltatime)
{
...
void Backgrounds::Draw(sf::RenderWindow& window)
{
	window.draw(body[0]);
	window.draw(body[1]);
}